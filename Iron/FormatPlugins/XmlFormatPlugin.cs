    ...
    public class XmlFormatPlugin : FormatPlugin
    {
        ...
        void OtoiReadNode(XmlNode Node, XmlWriter XW)
        {
            if (Node.NodeType == XmlNodeType.Element)
            {
                ...
                XW.WriteAttributeString("oname", Node.Name);
                XW.WriteAttributeString("local_name", Node.LocalName);
                XW.WriteAttributeString("prefix", Node.Prefix);
                XW.WriteAttributeString("namespace", Node.NamespaceURI);
                    ...
                    if (IsNonNameSpaceAttributesAvailable)
                    {
                        ...
                        foreach (XmlAttribute XA in Node.Attributes)
                        {
                            if (!(XA.Name == "xmlns" || XA.Name.StartsWith("xmlns:")))
                            {
                                ...
                                XW.WriteAttributeString("oname", XA.Name);
                                XW.WriteAttributeString("local_name", XA.LocalName);
                                XW.WriteAttributeString("prefix", XA.Prefix);
                                //XW.WriteValue(XA.Value);
                                this.WriteValue(XW, XA.Value);
                                XW.WriteEndElement();
                            }
                        }
                        XW.WriteEndElement();
                ...
                if (Node.HasChildNodes)
                {
                    if (Node.ChildNodes.Count == 1 && Node.ChildNodes[0].NodeType == XmlNodeType.Text)
                    {
                        ...
                        this.WriteValue(XW, Node.ChildNodes[0].InnerText);
                        XW.WriteEndElement();
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    {
                        XW.WriteAttributeString("empty_tag", "1");
                    }
                    //XW.WriteValue("");
                    this.WriteValue(XW, "");
                    XW.WriteEndElement();
        ...
        void ItooReadNode(XmlNode Node, XmlWriter XW)
        {
            if (Node.NodeType == XmlNodeType.Element)
            {
                ...
                {
                    XW.WriteComment(Tools.XmlSafe(Tools.Base64Decode(Node.InnerText)));
                }
                    ...
                    foreach (XmlNode ChildNode in Node.ChildNodes)
                    {
                        if (ChildNode.Name == "attributes")
                        {
                            foreach (XmlNode AttrNode in ChildNode.ChildNodes)
                            {
                                ...
                                XW.WriteValue(Tools.XmlSafe(Tools.Base64Decode(AttrNode.InnerText)));
                        ...
                        else if (ChildNode.Name == "value")
                        {
                            ...
                            {
                                XW.WriteValue(Tools.XmlSafe(Tools.Base64Decode(ChildNode.InnerText)));
                            }
                        }
                    }
                    XW.WriteEndElement();
        ...
        string GetXmlDeclaration(string XML)
        {
            ...
            try
            {
                ...
                XR.Read();
                while (XR.NodeType == XmlNodeType.Whitespace || XR.NodeType == XmlNodeType.SignificantWhitespace)
                {
                    XR.Read();
                }
