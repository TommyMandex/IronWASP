...
namespace IronWASP
{
    public class ModUiTools
    {
        ...
        static string XmlFromControl(Control C)
        {
            ...
            switch (C.GetType().Name)
            {
                ...
                case (ModUiTags.ModPanel):
                    ...
                    break;
                case (ModUiTags.ModSplitContainer):
                    SB.Append(XmlFromModSplitContainer(C as ModSplitContainer));
                    break;
        ...
        static string XmlFromModSplitContainer(ModSplitContainer C)
        {
            StringBuilder SB = new StringBuilder();
            SB.Append(StartTag(ModUiTags.ModSplitContainer));
            SB.Append(XmlFromControlNameProperty(C));
            SB.Append(StartTag(ModUiTags.Properties));
            //basic properties
            SB.Append(XmlFromBasicProperties(C));
            SB.Append(XmlFromSplitContainerFixedPanel(C.FixedPanel));
            SB.Append(XmlFromSplitContainerIsSplitterFixed(C.IsSplitterFixed));
            SB.Append(XmlFromSplitContainerOrientation(C.Orientation));
            SB.Append(XmlFromSplitContainerSplitterDistance(C.SplitterDistance));
            SB.Append(EndTag(ModUiTags.Properties));
            //child controls
            SB.Append(XmlFromSplitContainerPanelOneChildrenProperty(C.Panel1));
            SB.Append(XmlFromSplitContainerPanelTwoChildrenProperty(C.Panel2));
            SB.Append(XmlFromEventHandlers(C.EventHandlers));

            SB.Append(EndTag(ModUiTags.ModSplitContainer));
        ...
        static string XmlFromSplitContainerFixedPanel(FixedPanel SplitContPanel)
        {
            return XmlFromTagValue(ModUiTags.FixedPanel, SplitContPanel);
        }
        static string XmlFromSplitContainerIsSplitterFixed(bool IsSplitterFixed)
        {
            return XmlFromTagValue(ModUiTags.IsSplitterFixed, IsSplitterFixed);
        }
        static string XmlFromSplitContainerOrientation(Orientation SplitContOrientation)
        {
            return XmlFromTagValue(ModUiTags.Orientation, SplitContOrientation);
        }
        static string XmlFromSplitContainerPanelOneChildrenProperty(SplitterPanel PanelOne)
        {
            return XmlFromSplitContainerPanelChildrenProperty(PanelOne, ModUiTags.PanelOne);
        }
        static string XmlFromSplitContainerPanelTwoChildrenProperty(SplitterPanel PanelTwo)
        {
            return XmlFromSplitContainerPanelChildrenProperty(PanelTwo, ModUiTags.PanelTwo);
        }
        static string XmlFromSplitContainerPanelChildrenProperty(SplitterPanel Panel, string PanelName)
        {
            StringBuilder SB = new StringBuilder();
            SB.Append(StartTag(PanelName));
            SB.Append(XmlFromControlChildrenProperty(Panel.Controls));
            SB.Append(EndTag(PanelName));
            return SB.ToString();
        }
        static string XmlFromSplitContainerSplitterDistance(int SplitterDistance)
        {
            return XmlFromTagValue(ModUiTags.SplitterDistance, SplitterDistance);
        }
        ...
        ModCodeAndControlHolder ConvertControlPropertiesEventHandlersChildrenToCode(XmlNode Node, string ControlName, Control C)
        {
            ...
            XmlNode PanelOneNode = null;
            XmlNode PanelTwoNode = null;
            foreach (XmlNode SubNode in Node.ChildNodes)
            {
                switch (SubNode.Name)
                {
                    ...
                    case (ModUiTags.Children):
                        ...
                        break;
                    case (ModUiTags.PanelOne):
                        PanelOneNode = SubNode;
                        break;
                    case (ModUiTags.PanelTwo):
                        PanelTwoNode = SubNode;
                        break;
                    default:
                        continue;
                }
            ...
            if (PanelOneNode != null)
            {
                NodeResults = ConvertControlChildXmlNodesToCode(PanelOneNode.ChildNodes[0], string.Format("{0}.Panel1", ControlName), C);
                PyCode.Append(NodeResults.PyCode);
                RbCode.Append(NodeResults.RbCode);
            }
            if (PanelTwoNode != null)
            {
                NodeResults = ConvertControlChildXmlNodesToCode(PanelTwoNode.ChildNodes[0], string.Format("{0}.Panel2", ControlName), C);
                PyCode.Append(NodeResults.PyCode);
                RbCode.Append(NodeResults.RbCode);
            }
            ModCodeAndControlHolder Result = new ModCodeAndControlHolder();
        ...
        ModCodeAndControlHolder ConvertControlPropertyNodeToCode(XmlNode PropertyNode, string ControlName, Control C)
        {
            ...
            switch (PropertyNode.Name)
            {
                ...
                case (ModUiTags.TabPages):
                    ...
                    break;
                case (ModUiTags.Orientation):
                    SetSplitContainerOrientationInCode(PyCode, RbCode, ControlName, C, PropertyNode);
                    break;
                case (ModUiTags.IsSplitterFixed):
                    SetSplitContainerIsSplitterFixedInCode(PyCode, RbCode, ControlName, C, PropertyNode);
                    break;
                case (ModUiTags.FixedPanel):
                    SetSplitContainerFixedPanelInCode(PyCode, RbCode, ControlName, C, PropertyNode);
                    break;
                case (ModUiTags.SplitterDistance):
                    SetSplitContainerSplitterDistanceInCode(PyCode, RbCode, ControlName, C, PropertyNode);
                    break;
            }
            ModCodeAndControlHolder Result = new ModCodeAndControlHolder();
            Result.PyCode = PyCode.ToString();
        ...
        static void SetBorderStyleInCode(StringBuilder PyCode, StringBuilder RbCode, string ControlName, Control C, XmlNode Node)
        {
            ...
            switch (C.GetType().Name)
            {
                ...
                case ("ModLabel"):
                    ...
                    break;
                case ("ModSplitContainer"):
                    (C as ModSplitContainer).BorderStyle = ModUiTools.GetBorderStyleDefinition(BorderStyle);
                    break;
                default:
                    return;
            }
        ...
        static void SetSplitContainerOrientationInCode(StringBuilder PyCode, StringBuilder RbCode, string ControlName, Control C, XmlNode Node)
        {
            PyCode.AppendLine(string.Format("{0}.Orientation = ModUiTools.GetSplitOrientation('{1}')", ControlName, Node.InnerText));
        ...
        static void SetSplitContainerFixedPanelInCode(StringBuilder PyCode, StringBuilder RbCode, string ControlName, Control C, XmlNode Node)
        {
            PyCode.AppendLine(string.Format("{0}.FixedPanel = ModUiTools.GetFixedPanel('{1}')", ControlName, Node.InnerText));
            RbCode.AppendLine(string.Format("{0}.FixedPanel = ModUiTools.get_fixed_panel('{1}')", ControlName, Node.InnerText));
            (C as ModSplitContainer).FixedPanel = ModUiTools.GetFixedPanel(Node.InnerText);
        }
        static void SetSplitContainerSplitterDistanceInCode(StringBuilder PyCode, StringBuilder RbCode, string ControlName, Control C, XmlNode Node)
        {
            int SplitterDistance = Int32.Parse(Node.InnerText);
            PyCode.AppendLine(string.Format("{0}.SplitterDistance = {1}", ControlName, SplitterDistance));
            RbCode.AppendLine(string.Format("{0}.SplitterDistance = {1}", ControlName, SplitterDistance));
            (C as ModSplitContainer).SplitterDistance = SplitterDistance;
        }
        static void SetSplitContainerIsSplitterFixedInCode(StringBuilder PyCode, StringBuilder RbCode, string ControlName, Control C, XmlNode Node)
        {
            bool IsFixed = (Node.InnerText == "True");
            PyCode.AppendLine(string.Format("{0}.IsSplitterFixed = {1}", ControlName, PyBool(IsFixed)));
            RbCode.AppendLine(string.Format("{0}.IsSplitterFixed = {1}", ControlName, RbBool(IsFixed)));
            (C as ModSplitContainer).IsSplitterFixed = IsFixed;
        }
        void SetSplitContainerPanelTwoInCode(StringBuilder PyCode, StringBuilder RbCode, string ControlName, Control C, XmlNode Node)
        {
            if (!Node.HasChildNodes) return;
            foreach (XmlNode TabPageNode in Node.ChildNodes)
            {
            ...
            foreach (XmlNode ChildNode in ParentNode.ChildNodes)
            {
                ...
                switch (ChildNode.Name)
                {
                    ...
                    case ("ModPanel"):
                        ...
                        break;
                    case ("ModSplitContainer"):
                        PyCode.AppendLine(string.Format("{0} = ModSplitContainer()", ControlName));
                        RbCode.AppendLine(string.Format("{0} = ModSplitContainer.new()", ControlName));
                        try
                        {
                            C = (ModSplitContainer)ModUiDesigner.IDH.CreateComponent(typeof(ModSplitContainer), ControlName);
                        }
                        catch { C = new ModSplitContainer(); }
                        break;
                    case ("ModTabControl"):
                ...
                {
                    CPECC.Control.Parent = ((SplitContainer)ParentControl).Panel1;
                }
                else if (ParentName.EndsWith(".Panel2"))
                {
                    CPECC.Control.Parent = ((SplitContainer)ParentControl).Panel2;
                }
                else
                {
        ...
        public static Orientation GetSplitOrientation(string OrientationString)
        {
            switch (OrientationString)
            {
                ...
                default:
                    throw new Exception(string.Format("Invalid TextAlignment - {0}", OrientationString));
            }
        }
        public static FixedPanel GetFixedPanel(string FixedPanelName)
        {
            switch (FixedPanelName)
            {
                case ("None"):
                    return FixedPanel.None;
                case ("Panel1"):
                    return FixedPanel.Panel1;
                case ("Panel2"):
                    return FixedPanel.Panel2;
                default:
                    throw new Exception(string.Format("Invalid TextAlignment - {0}", FixedPanelName));
            }
        }
    ...
    public static class ModUiTags
    {
        ...
        public const string ModSplitContainer = "ModSplitContainer";
        ...
        //splitcontainer
        public const string FixedPanel = "fixed_panel";
        public const string IsSplitterFixed = "is_splitter_fixed";
        public const string Orientation = "orientation";
        public const string SplitterDistance = "splitter_distance";
        public const string PanelOne = "panel_one";
        public const string PanelTwo = "panel_two";
        #endregion
