...
namespace IronWASP
{
    public class Analyzer
    {
        ...
        public static Reflection GetReflectionsFor(string Input, string ResString)
        {
            ...
            Regex Reg = new Regex(String.Format(@"\W{0}\W", Regex.Escape(Input)), RegexOptions.Multiline | RegexOptions.IgnoreCase | RegexOptions.IgnorePatternWhitespace);

            MatchCollection MatchResults = Reg.Matches(ResString);
            ...
            foreach (Match M in MatchResults)
            {
                if (M.Success)
                {
                    ...
                    if (SubStringStart + SubStringLength >= ResString.Length)
                        Substring = ResString.Substring(SubStringStart);
                    else
                        Substring = ResString.Substring(SubStringStart, SubStringLength);

                    Substring = Substring.Replace(M.Value, M.Value.Replace(Input, string.Format("<i<hlo>>{0}<i</hlo>>", Input)));
