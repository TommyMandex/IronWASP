...
namespace IronWASP
{
    public partial class Main : Form
    {
        ...
        {          
            ...
            IronUI.LF.ShowLoadMessage("Loading core REST API registrations....");
            try
            {
                RestApi.CoreApiLoader.LoadCoreCustomApiRegistration();
            }
            catch (Exception Exp)
            {
                IronException.Report("Error loading core API registrations", Exp.Message, Exp.StackTrace);
            }

            IronUI.LF.ShowLoadMessage("Loading custom API registrations from Python script....");
            try
            {
                RestApi.CustomApiLoader.LoadCustomApiRegistrationFromPythonScript();
            }
            catch (Exception Exp)
            {
                IronException.Report("Error loading custom API registrations from Python script", Exp.Message, Exp.StackTrace);
            }
            IronUI.LF.ShowLoadMessage("Loading custom API registrations from Ruby script....");
            try
            {
                RestApi.CustomApiLoader.LoadCustomApiRegistrationFromRubyScript();
            }
            catch (Exception Exp)
            {
                IronException.Report("Error loading custom API registrations from Ruby script", Exp.Message, Exp.StackTrace);
            }
            ...
            IronUI.LF.ShowLoadMessage("Reading JS files for insertion....");
            try
            {
                JavaScriptTracer.LoadJsToInset();
            }
            catch (Exception Exp) 
            {
                IronException.Report("Error loading JS files to insert", Exp); 
            }
            try
            {
                JavaScriptTracer.StartAnalyzer();
            }
            catch (Exception Exp)
            {
                IronException.Report("Unable to start JavaScript Trace Analyer", Exp);
            }
        ...
        private void ProxySendBtn_Click(object sender, EventArgs e)
        {
            ...
            if (SessionID.Length > 0)
            {
                ...
                lock(IronProxy.InterceptedSessions)
                {
                    ...
                    {
                        IronException.Report("Error retriving session intercepted by proxy", Exp);
                    }
                }
        ...
        private void InterceptRequestCB_CheckedChanged(object sender, EventArgs e)
        {
            if(IronProxy.InterceptRequest)
            {
                ...
                if (IronProxy.SessionsQ.Count == 0)
                {
                    ProxyBaseSplit.Panel1.BackColor = Color.White;
                    ProxySendBtn.Enabled = false;
                    ProxyDropBtn.Enabled = false;
        ...
        {
            //ShowSelectedProxyLog();
        }

        void ShowSelectedProxyLog()
        {
        ...
        {
            //ShowSelectedShellLog();
        }

        void ShowSelectedShellLog()
        {
        ...
        private void UpdateBodyGridForFormat(FormatPlugin Plugin, bool CheckStatus)
        {
            ...
            for (int i = 0; i < InjectionPoints.GetLength(0); i++)
            {
                try
                {
                    ConfigureScanRequestBodyTypeFormatPluginGrid.Rows.Add(new object[] { CheckStatus, InjectionPoints[i, 0], Tools.Base64Decode(InjectionPoints[i, 1]) });
                }
                catch
                {
        ...
        {
            //ShowSelectedSitemapLog();
        }

        void ShowSelectedSitemapLog()
        {
        ...
        {
            //ShowSelectedProbeLog();   
        }

        void ShowSelectedProbeLog()
        {
        ...
        private void main_tab_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (main_tab.SelectedTab.Name.Equals("mt_logs"))
                {
                    if (!Docs.DocForLog.WasShown)
                    {
                        Docs.DocForLog.WasShown = true;
                        Docs.DocForLog DFL = new Docs.DocForLog();
                        DFL.Show();
                    }
                }
            }
        ...
        {
            //ShowSelectedOtherLog();
        }

        void ShowSelectedOtherLog()
        {
       ...
       //}
        ...
        {
            LogAnalyzerAndTester LoAn = new LogAnalyzerAndTester();
            LoAn.Show();
        }
        ...
        private void LoginCSRFTokenSequenceRecorderToolStripMenuItem_Click_1(object sender, EventArgs e)
        {
            Recording.Recorder.OpenWindow();
        }

        private void LoadRecodedSequenceToolStripMenuItem_Click(object sender, EventArgs e)
        {
            Recording.RecordingLoader.OpenWindow();
        }

        private void BrowserBasedCrawlerToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                Tools.RunWith(Config.RootDir + "\\WebDriverCrawler.exe", IronProxy.Port.ToString());
            }
            catch(Exception Exp) 
            {
                IronException.Report("Error opening WebDriverCrawler", Exp);
                MessageBox.Show("Error opening the crawler.");
            }
        }

        private void TestForCSRFProtectionToolStripMenuItem_Click(object sender, EventArgs e)
        {
            Hawas.CsrfTester.OpenWindow();
        }

        private void WorkflowScannerToolStripMenuItem_Click(object sender, EventArgs e)
        {
            Workflow.WorkflowScannerWindow.OpenWindow();
        }

        private void DOMXSSAnalyzerToolStripMenuItem_Click(object sender, EventArgs e)
        {
            Hawas.DomXssAnalyzer.OpenWindow();
        }

        private void TestForBrokenAuthenticationToolStripMenuItem_Click(object sender, EventArgs e)
        {
            Hawas.BrokenAuthTester.OpenWindow();
        }

        private void TestForHiddenParameterToolStripMenuItem_Click(object sender, EventArgs e)
        {
            Hawas.HiddenParameterGuesser.OpenWindow();
        }

        private void TestForPrvilegeEscalationToolStripMenuItem_Click(object sender, EventArgs e)
        {
            Hawas.PrivillegeEscalationTester.OpenWindow();
        }

        long TimeWhenLogGridIndexWasLastChanged = 0;
        private void ProxyLogGrid_SelectionChanged(object sender, EventArgs e)
        {
        ...
        private void ScanLogGrid_SelectionChanged(object sender, EventArgs e)
        {
            if (DateTime.Now.TimeOfDay.TotalMilliseconds > TimeWhenLogGridIndexWasLastChanged + 200)
            {
                ShowSelectedScanLog();
        ...
        private void TestLogGrid_SelectionChanged(object sender, EventArgs e)
        {
            if (DateTime.Now.TimeOfDay.TotalMilliseconds > TimeWhenLogGridIndexWasLastChanged + 200)
            {
                ShowSelectedMTLog();
        ...
        private void ShellLogGrid_SelectionChanged(object sender, EventArgs e)
        {
            if (DateTime.Now.TimeOfDay.TotalMilliseconds > TimeWhenLogGridIndexWasLastChanged + 200)
            {
                ShowSelectedShellLog();
        ...
        private void ProbeLogGrid_SelectionChanged(object sender, EventArgs e)
        {
            if (DateTime.Now.TimeOfDay.TotalMilliseconds > TimeWhenLogGridIndexWasLastChanged + 200)
            {
                ShowSelectedProbeLog();
        ...
        private void OtherLogGrid_SelectionChanged(object sender, EventArgs e)
        {
            if (DateTime.Now.TimeOfDay.TotalMilliseconds > TimeWhenLogGridIndexWasLastChanged + 200)
            {
                ShowSelectedOtherLog();
        ...
        private void SiteMapLogGrid_SelectionChanged(object sender, EventArgs e)
        {
            if (DateTime.Now.TimeOfDay.TotalMilliseconds > TimeWhenLogGridIndexWasLastChanged + 200)
            {
                ShowSelectedSitemapLog();
        ...
        private void LogsFromAnotherIronWASPProjectFileToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ProjectFileOpenDialog.InitialDirectory = Config.RootDir + "\\log\\";
            if (IronUI.IsImportFormOpen())
            {
            ...
            while (ProjectFileOpenDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    FileInfo Info = new FileInfo(ProjectFileOpenDialog.FileName);
                    Import.ImportLogsFromIronwaspProjectFile(Info.Directory.FullName);
                    break;
                }
            }
        }

        private void MainDocLL_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            Docs.DocForNewUser DFNU = new Docs.DocForNewUser();
            DFNU.Show();
        }

        private void ProxyOpenBrowserLL_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            MessageBox.Show("To launch a browser pre-configued with this Proxy, from the main menu launch 'Tools'-> 'Browser based Crawler' and click on the 'Open Manual Crawler' button.");
        }

        private void webSocketMessageFormaterToolStripMenuItem_Click(object sender, EventArgs e)
        {
            Hawas.WebsocketMessageAnalyzer.OpenWindow();
        }
        ...
        private void webSocketClientToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                Tools.RunWith(Config.RootDir + "\\WebsocketClient.exe", IronProxy.Port.ToString());
            }
            catch (Exception Exp)
            {
                IronException.Report("Error opening Websocket Client", Exp);
                MessageBox.Show("Error opening the Websocket Client");
            }
        }

        private void JsTraceEnableCB_CheckedChanged(object sender, EventArgs e)
        {
            JavaScriptTracer.InjectJavaScript = JsTraceEnableCB.Checked;
        }

        private void JsTraceReloadFilesLL_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            try
            {
                JavaScriptTracer.LoadJsToInset();
            ...
            {
                IronException.Report("Error reloading JS tracing files", Exp);
            }
        }

        private void JsTraceConfigCBS_CheckedChanged(object sender, EventArgs e)
        {
            JavaScriptTracer.MonitorEval = JsTraceMonitorEvalCB.Checked;
            JavaScriptTracer.MonitorSetTimeout = JsTraceMonitorSetTimeoutCB.Checked;
            JavaScriptTracer.MonitorSetInterval = JsTraceMonitorSetIntervalCB.Checked;
            JavaScriptTracer.MonitorFunctionMethods = JsTraceMonitorFunctionMethodCB.Checked;
            JavaScriptTracer.MonitorUserDefinedMethods = JsTraceMonitorNonNativeMethodCB.Checked;

            JavaScriptTracer.MonitorInnerHtmlAssignments = JsTraceMonitorInnerHtmlCB.Checked;

            JavaScriptTracer.MonitorXhr = JsTraceMonitorXhrCB.Checked;
        }

        private void JsTraceResultsTab_SelectedIndexChanged(object sender, EventArgs e)
        {
            
        }

        private void JsTraceResultsObservationsGrid_SelectionChanged(object sender, EventArgs e)
        {
            try
            {
                if (JsTraceResultsObservationsGrid.SelectedRows != null && JsTraceResultsObservationsGrid.SelectedRows.Count == 1)
                {
                    IronUI.ShowJsTraceResult(JsTraceResultsObservationsGrid.SelectedRows[0].Cells["JSTRObservationDetailsClmn"].Value.ToString());
                }
            }
            catch(Exception Exp) 
            {
                IronException.Report("Error show selected JS Trace result", Exp);
            }
        }

        private void JsTraceResultsMethodCallsGrid_SelectionChanged(object sender, EventArgs e)
        {
            try 
            { 
                if (JsTraceResultsMethodCallsGrid.SelectedRows != null && JsTraceResultsMethodCallsGrid.SelectedRows.Count == 1)
                {
                    IronUI.ShowJsTraceResult(JsTraceResultsMethodCallsGrid.SelectedRows[0].Cells["JSTRNativeMethodCallDetailsClmn"].Value.ToString());
                }
            }
            catch (Exception Exp)
            {
        ...
        private void JsTraceResultsDomChangesGrid_SelectionChanged(object sender, EventArgs e)
        {
            try 
            { 
                if (JsTraceResultsDomChangesGrid.SelectedRows != null && JsTraceResultsDomChangesGrid.SelectedRows.Count == 1)
                {
                    IronUI.ShowJsTraceResult(JsTraceResultsDomChangesGrid.SelectedRows[0].Cells["JSTRDomChangesDetailsClmn"].Value.ToString());
                }
            }
            catch (Exception Exp)
            {
        ...
        private void JsTraceResultsAjaxCallsGrid_SelectionChanged(object sender, EventArgs e)
        {
            try
            { 
                if (JsTraceResultsAjaxCallsGrid.SelectedRows != null && JsTraceResultsAjaxCallsGrid.SelectedRows.Count == 1)
                {
                    IronUI.ShowJsTraceResult(JsTraceResultsAjaxCallsGrid.SelectedRows[0].Cells["JSTRAjaxCallsDetailsClmn"].Value.ToString());
                }
            }
            catch (Exception Exp)
            {
        ...
        private void JsTraceResultsObservationsGrid_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (JsTraceResultsObservationsGrid.SelectedRows != null && JsTraceResultsObservationsGrid.SelectedRows.Count == 1)
        ...
        private void JsTraceResultsMethodCallsGrid_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (JsTraceResultsMethodCallsGrid.SelectedRows != null && JsTraceResultsMethodCallsGrid.SelectedRows.Count == 1)
        ...
        private void JsTraceResultsDomChangesGrid_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (JsTraceResultsDomChangesGrid.SelectedRows != null && JsTraceResultsDomChangesGrid.SelectedRows.Count == 1)
        ...
        private void JsTraceResultsAjaxCallsGrid_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (JsTraceResultsAjaxCallsGrid.SelectedRows != null && JsTraceResultsAjaxCallsGrid.SelectedRows.Count == 1)
