...
namespace IronWASP
{
    public class Scanner
    {
        ...
        //Workflow scanning
        public int IdOfWorkflowToScan = 0;
        public Analysis.LogAssociations WorkFlowLogAssociations = null;
        public int IndexOfRequestToScanInWorkFlowLogAssociations = 0;

        //Login Recording
        public Recording.Recording LoginAndCsrfRecording = null;
        ...
        public int LaunchScan()
        {
            ...
            if (this.LoginAndCsrfRecording != null)
            {
            ...
            {
                IronDB.UpdateScan(ScanID, OriginalRequest, "Queued", this.GetInjectionString(), this.BodyFormat.Name, this.GetScanPluginsString(), this.SessionHandler.Name, this.IdOfWorkflowToScan , this.IndexOfRequestToScanInWorkFlowLogAssociations, 0);
            }
        ...
        internal void StartScan(int ScanID)
        {
            ...
            if (this.LoginAndCsrfRecording != null)
            {
                IronDB.UpdateScan(this.ScanID, this.OriginalRequest, "Queued", this.GetInjectionString(), this.BodyFormat.Name, this.GetScanPluginsString(), this.SessionHandler.Name, this.IdOfWorkflowToScan, this.IndexOfRequestToScanInWorkFlowLogAssociations, this.LoginAndCsrfRecording.WorkflowId);
            }
            else
            {
                IronDB.UpdateScan(this.ScanID, this.OriginalRequest, "Queued", this.GetInjectionString(), this.BodyFormat.Name, this.GetScanPluginsString(), this.SessionHandler.Name, this.IdOfWorkflowToScan, this.IndexOfRequestToScanInWorkFlowLogAssociations, 0);
            }
        ...
        private Response Inject(string RawPayload, int ReDoCount, bool PayloadLessInjection, bool Raw, int TimeOut)
        {
            ...
            if (this.LoginAndCsrfRecording != null)
            {
                this.TestRequest.SetCookie(this.LoginAndCsrfRecording.Cookies);
                if (!this.LoginAndCsrfRecording.IsLoggedIn())
                {
                    this.LoginAndCsrfRecording.DoLogin();
                    if (!this.LoginAndCsrfRecording.IsLoggedIn())
                    {
                ...
                if (this.LoginAndCsrfRecording.CsrfParameterName.Length > 0)
                {
                    if (this.TestRequest.Query.Has(this.LoginAndCsrfRecording.CsrfParameterName))
                    {
                        this.TestRequest.Query.Set(this.LoginAndCsrfRecording.CsrfParameterName, this.LoginAndCsrfRecording.GetCsrfToken());
                    }
                    else if (this.TestRequest.Body.Has(this.LoginAndCsrfRecording.CsrfParameterName))
                    {
                        this.TestRequest.Body.Set(this.LoginAndCsrfRecording.CsrfParameterName, this.LoginAndCsrfRecording.GetCsrfToken());
                    }
                }
            }

            Analysis.LogReplayer WorkFlowReplayer = null;
            if (WorkFlowLogAssociations != null)
            {
                WorkFlowReplayer = new Analysis.LogReplayer(WorkFlowLogAssociations);
                for (int i = 0; i <= IndexOfRequestToScanInWorkFlowLogAssociations; i++)
                {
                    if (i == IndexOfRequestToScanInWorkFlowLogAssociations)
                    {
                        this.TestRequest = WorkFlowReplayer.PrepareStepForPlay();
                    }
                    else
                    {
                        WorkFlowReplayer.PrepareStepForPlay();
                        WorkFlowReplayer.PlayStep();
                    }
                }
            }
            else
            {
                ...
                if (SessionHandler.Name.Length > 0)
                {
            ...
            Response ResponseFromInjection = null;
            if (WorkFlowLogAssociations != null)
            {
                this.TestResponse = WorkFlowReplayer.PlayStep();
                while (WorkFlowReplayer.HasMoreStepsToPlay())
                {
                    WorkFlowReplayer.PrepareStepForPlay();
                    WorkFlowReplayer.PlayStep();
                }
                ResponseFromInjection = this.TestResponse;
            }
            else
            {
                if (TimeOut > 0)
                ...
                if (SessionHandler.Name.Length > 0)
                {
                    ...
                    ResponseFromInjection = SessionHandler.DoAfterSending(this.TestResponse, this.TestRequest);
                }
                else
                {
                    ResponseFromInjection = this.TestResponse;
        ...
        string RemoveCustomInjectionPointMarkers(string BodyStr)
        {
            return BodyStr.Replace(this.CustomInjectionPointStartMarker, "").Replace(this.CustomInjectionPointEndMarker, "");
        }
        ...
        List<string> SplitAtCustomInjectionPoints()
        {
            ...
            {
                this.BodyCustomInjectionParts = SplitAtCustomInjectionPoints(this.CurrentRequest.BodyString, this.CustomInjectionPointStartMarker, this.CustomInjectionPointEndMarker);
            }
            else
            {
