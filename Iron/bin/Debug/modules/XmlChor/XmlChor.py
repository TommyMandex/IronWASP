...
class XmlChor(Module):
  ...
  def StartModuleOnSession(self, sess):
    ...
    while f.HasMore():
      ...
      try:
        self.console.PrintLine("[*] Checking if {0} parameter can be exploited automatically.".format(f.InjectedParameter))
        prevalue=self.check_quoteandoperator(preInjectVal)
	  
        #prefix
        if(prevalue!=""):
          self.console.PrintLine("[*] Parameter is exploitable, will perform exploitation using {0} as the quote and prefix as : {1} ".format(prevalue["quote"], prevalue["prefix"]))
        else:
          self.console.PrintLine("[*] {0} parameter is not exploitable".format(f.InjectedParameter))
          continue
        ...
        IronException.Report("Exception in XmlChor", str(exp))
        self.console.PrintLine("[*] Something went wrong, unable to continue. Error details: {0}".format(str(exp)))
        continue
      ...
      root_array=[1]
      self.xml = ""
      self.console.PrintLine("[*] Extracting backend XML through XPATH Injection.......")
      try:
      ...
      try:
        self.console.PrintLine("[*] Extracted XML is: \r\n {0}".format(Tools.BeautifyXml(self.xml)))
      except:
        pass
  ...
  def get_XML_node_and_count(self, root_node_index,prevalue):
    if(self.ex.IsBoolWorking()):
     # self.console.PrintLine("will now get bool value with double quote and: "+prevalue)
      ...
      self.xml = self.xml + "<" + str(node_name)
      ...
      if(attr_count>0):
        ...
        for i in range(attr_count):
          ...
          self.xml = self.xml + str(attr_name) + " = " + str(attr_value)
      ...
      self.xml = self.xml + first_node_value + "</"+ str(node_name) + ">"
