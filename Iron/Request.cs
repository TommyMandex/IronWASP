...
namespace IronWASP
{
    public class Request
    {
        ...
        BodyFormatType formatType = BodyFormatType.UrlFormEncoded;
        string BodyStringHash = "";
        ...
        public bool IsBinary
        {
            get
            {
                return (this.formatType == BodyFormatType.UnknownBinary);
            }
        }
        ...
        public bool IsNormal
        {
            get
            {
                return (this.formatType == BodyFormatType.UrlFormEncoded);
            }
        }
        ...
        public bool IsXml
        {
            get
            {
                return (this.formatType == BodyFormatType.Xml || this.formatType == BodyFormatType.Soap);
            }
        }
        ...
        public bool IsSoap
        {
            get
            {
                return (this.formatType == BodyFormatType.Soap);
            }
        }
        ...
        public bool IsJson
        {
            get
            {
                return (this.formatType == BodyFormatType.Json);
            }
        }
        ...
        public bool IsMultiPart
        {
            get
            {
                return (this.formatType == BodyFormatType.Multipart);
            }
        }

        public BodyFormatType BodyType
        {
            get
            {
       ...
       //if the file extension is longer than 8 characters or has any non-alpha characters then return ""
        ...
        public Response Send()
        {
            ...
            //}
            if (OverrideHostTo.Length > 0)
            {
                Flags["x-overrideHost"] = this.Host;
                this.Host = OverrideHostTo;
            }
            ...
            if ((Res.Code == 502 || Res.Code == 504) && Res.Status.StartsWith("Fiddler - "))
            {
        ...
        void CheckBodyFormat()
        {
            if (!BodyStringHash.Equals(Tools.MD5(this.BodyString)))
            {
                this.formatType = FormatPlugin.GetFormatType(this);
                this.BodyStringHash = Tools.MD5(this.BodyString);
            }
        }
        ...
        void AbsorbFullURL(string FullURL)
        {
            ...
            {
                if ((HostAndPath.Contains("/") && (HostAndPath.IndexOf('?') < HostAndPath.IndexOf('/'))) || !HostAndPath.Contains("/"))
                {
                    string[] HostAndPathParts = HostAndPath.Split(new char[] { '?' }, 2);
                    if (HostAndPathParts.Length == 2)
                    {
                        HostAndPath = string.Format("{0}/?{1}", HostAndPathParts[0], HostAndPathParts[1]);
                    }
                }
            }
            else if (!HostAndPath.Contains("/"))
            {
        ...
        public string RelativeUrlToAbsoluteUrl(string RelativeUrl)
        {
            string AbsoluteUrl = Crawler.NormalizeUrl(this, RelativeUrl);
            try
            {
