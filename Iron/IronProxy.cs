...
namespace IronWASP
{
    public class IronProxy
    {
        ...
        static List<string> ApiCallHostNames = new List<string>();
        ...
        internal static bool Start()
        {
            ...
            if (!EventHandlersAssigned)
            {
                ...
                Fiddler.FiddlerApplication.OnWebSocketMessage += delegate(object sender, Fiddler.WebSocketMessageEventArgs e)
                {
                    IronProxy.OnWebSocketMessage(sender, e);
                };
            ...
            else
            {
                ...
                else
                {
                    if (IronProxy.LoopBackOnly)
                    {
                        Fiddler.FiddlerApplication.Startup(IronProxy.Port, Fiddler.FiddlerCoreStartupFlags.Default & ~Fiddler.FiddlerCoreStartupFlags.AllowRemoteClients & ~Fiddler.FiddlerCoreStartupFlags.ChainToUpstreamGateway & ~Fiddler.FiddlerCoreStartupFlags.RegisterAsSystemProxy);
                    }
                    else
                    {
                        Fiddler.FiddlerApplication.Startup(IronProxy.Port, Fiddler.FiddlerCoreStartupFlags.Default & ~Fiddler.FiddlerCoreStartupFlags.ChainToUpstreamGateway & ~Fiddler.FiddlerCoreStartupFlags.RegisterAsSystemProxy);
                    }
        ...
        internal static void BeforeRequest(Fiddler.Session Sess)
        {
            ...
            if (IsCallToIronwaspApi(Sess))
            {
                ApiCallHandler.Handle(Sess);
                return;
            }
            ...
            if (!IrSe.FiddlerSession.isFlagSet(Fiddler.SessionFlags.RequestGeneratedByFiddler))
            {
                ...
                JavaScriptTracer.ProcessSession(IrSe);
                IrSe.OriginalRequest = IrSe.Request.GetClone(true);

                //If passive crawler is running then send this request for analysis
                PassiveCrawler.CapturedCrawlRequest(IrSe);

                //If recording is running then send this request for analysis to recorder
                Recording.Recorder.AddParameterValues(IrSe);

                //If workflow is being recorded then this is sent for analysis
                Workflow.WorkflowScanner.CheckWorkflowRequest(IrSe);
            ...
            if (IronProxy.UseUpstreamProxy)
            {
                ...
                if (IrSe.Request.Host.StartsWith("127.0.0.1:") || IrSe.Request.Host.StartsWith("localhost:") || IrSe.Request.Host.Equals("127.0.0.1") || IrSe.Request.Host.Equals("localhost"))
                {
                    ...
                    {
                        IrSe.FiddlerSession.oFlags.Add("x-overrideGateway", "DIRECT");        
                    }
                }
                else
                {
            ...
            else if (IronProxy.UseSystemProxyAsUpStreamProxy)
            {
                if (IrSe.Request.Host.StartsWith("127.0.0.1:") || IrSe.Request.Host.StartsWith("localhost:") || IrSe.Request.Host.Equals("127.0.0.1") || IrSe.Request.Host.Equals("localhost"))
                {
                        IrSe.FiddlerSession.oFlags.Add("x-overrideGateway", "DIRECT");
        ...
        internal static void BeforeResponse(Fiddler.Session Sess)
        {
            ...
            if (!IrSe.FiddlerSession.isFlagSet(Fiddler.SessionFlags.RequestGeneratedByFiddler))
            {
                ...
                //If passive crawler is running then send this response for analysis
                PassiveCrawler.CapturedCrawlResponse(IrSe);

                //If recording is going on then send this response to recorder for analysis
                Recording.Recorder.AddHiddenFieldValues(IrSe);

                //If workflow recording is going on then send this request for analysis
                Workflow.WorkflowScanner.CheckWorkflowResponse(IrSe);
        ...
        internal static void OnWebSocketMessage(object sender, Fiddler.WebSocketMessageEventArgs e)
        {
            Fiddler.Session Sess = (Fiddler.Session)sender;
            Session WsSess = new Session(Sess);
            int ID = Config.GetNewId("WebSocket");
            WsSess.Request.ID = ID;
            WsSess.Response.ID = ID;
            WsSess.Request.Source = "WebSocket";
            WsSess.Response.Source = "WebSocket";
            WsSess.Request.Headers.Set("IW-WS-SocketID", Sess.id.ToString());
            WsSess.Request.Headers.Set("IW-WS-SocketMsgID", e.oWSM.ID.ToString());
            if (e.oWSM.IsOutbound)
            {
                //WsSess.Request.BodyArray = new byte[e.oWSM.PayloadData.Length];
                //e.oWSM.PayloadData.CopyTo(WsSess.Request.BodyArray, 0);
                WsSess.Request.BodyArray = e.oWSM.PayloadAsBytes();
                IronUpdater.AddOtherSourceRequest(WsSess.Request);
            }
            else
            {
                IronUpdater.AddOtherSourceRequest(WsSess.Request);
                //WsSess.Response.BodyArray = new byte[e.oWSM.PayloadData.Length];
                //e.oWSM.PayloadData.CopyTo(WsSess.Response.BodyArray, 0);
                WsSess.Response.BodyArray = e.oWSM.PayloadAsBytes();
                IronUpdater.AddOtherSourceResponse(WsSess.Response);
            }

            if (CanInterceptRequest(WsSess))
            {
                /*
                WsSess.MSR = new ManualResetEvent(false);
                if (e.oWSM.IsOutbound)
                {
                    WsSess.FiddlerSession.state = Fiddler.SessionStates.HandTamperRequest;
                    InterceptedSessions.Add(WsSess.ID + "-Request", WsSess);
                }
                else
                {
                    WsSess.FiddlerSession.state = Fiddler.SessionStates.HandTamperResponse;
                    InterceptedSessions.Add(WsSess.ID + "-Response", WsSess);
                }
                IronUI.SendSessionToProxy(WsSess);
                if (e.oWSM.IsOutbound)
                {
                    InterceptedSessions[WsSess.ID + "-Request"].MSR.WaitOne();
                    InterceptedSessions.Remove(WsSess.ID + "-Request");
                    e.oWSM.SetPayload(WsSess.Request.BodyArray);
                }
                else
                {
                    InterceptedSessions[WsSess.ID + "-Response"].MSR.WaitOne();
                    InterceptedSessions.Remove(WsSess.ID + "-Response");
                    e.oWSM.SetPayload(WsSess.Response.BodyArray);
                }
                */
                if (e.oWSM.IsOutbound)
                {
        ...
        //This method uses a hack since FiddlerCore does not expose the _isDirectRequestToFiddler() method in the Session class.
        internal static bool IsCallToIronwaspApi(Fiddler.Session Sess)
        {
            if (Sess.oRequest.headers.Exists("X-Ironwasp-Api-Call"))
            {
                return true;
            }
            if (Sess.port != IronProxy.Port)
            {
                return false;
            }
            if (!Sess.url.Substring(Sess.url.IndexOf("/")).StartsWith("/IronWASP/API/", StringComparison.OrdinalIgnoreCase))
            {
                return false;
            }
            if (ApiCallHostNames.Contains(Sess.host))
            {
                return true;
            }
            Request EchoCheckReq = new Request(Sess.fullUrl);
            EchoCheckReq.Url = "/fiddler_echo_check";
            EchoCheckReq.Source = RequestSource.Stealth;
            try
            {
                Response EchoCheckResponse = EchoCheckReq.Send(1000);
                if (EchoCheckResponse.Status == "Fiddler Generated" && EchoCheckResponse.Html.Title == "Fiddler Echo Service")
                {
                    lock (ApiCallHostNames)
                    {
                        ApiCallHostNames.Add(Sess.host);
                    }
                    return true;
                }
                else
