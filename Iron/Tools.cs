...
namespace IronWASP
{
    public class Tools
    {
        static string[] UrlInvalidChars = new string[] { " ", "\t", "\r", "\n", "\"", "<", ">", "{", "}", "|", "\\", "^", "[", "]", "`" };
        ...
        public static string StripRtfTags(string Text)
        {
        ...
        public static string RtfSafe(string Text)
        {
            Text = Text.Replace("\r\n", "<i<br>>");
            Text = Text.Replace("\n", "<i<br>>");
            Text = Text.Replace("\\","\\\\");
        ...
        public static string UrlEncodeBytes(byte[] Input)
        {
            return HttpUtility.UrlEncode(Input);
        }
        ...
        public static byte[] HexToBytes(string Input)
        {
            ...
            foreach (string Byte in Bytes)
            {
                BytesList.Add((byte)Convert.ToInt32(Byte, 16));
            }
        ...
        {
            return Base64EncodeByteArray(Input);
        }

        public static string XmlSafe(string Input)
        {
            if (IsAllowedInXml(Input))
            {
                return Input;
            }
            else
            {
                ...
                foreach (char C in Input)
                {
                    ...
                    else
                    {
                        try
                        {
                            SB.Append(Tools.XmlEncode(C.ToString()));//Empty append might throw an exception
                        }
                        catch { }
                    }
                }
        ...
        public static bool IsAllowedInXml(string Input)
        {
            try
            {
                StringBuilder XB = new StringBuilder();
                XmlWriterSettings Settings = new XmlWriterSettings();
                using (XmlWriter XW = XmlWriter.Create(XB, Settings))
                {
                    XW.WriteStartElement("xml");
                    XW.WriteValue(Input);
                    XW.WriteEndElement();
        ...
        {
            return Base64DecodeToByteArray(Input);
        }
        ...
        public static string SHA256(string Input)
        {
            return SHA256Bytes(ToByteArray(Input));
        }

        public static string SHA256Bytes(byte[] InBa)
        {
        ...
        public static string SHA512(string Input)
        {
            return SHA512Bytes(ToByteArray(Input));
        }

        public static string SHA512Bytes(byte[] InBa)
        {
        ...
        public static string SHA1(string Input)
        {
            return SHA1Bytes(ToByteArray(Input));
        }

        public static string SHA1Bytes(byte[] InBa)
        {
        ...
        public static string SHA384(string Input)
        {
            return SHA384Bytes(ToByteArray(Input));
        }

        public static string SHA384Bytes(byte[] InBa)
        {
        ...
        public static string md5(string Input, string EncodingName)
        {
            return MD5(Input, EncodingName);
        }

        public static string md5_bytes(byte[] Input)
        {
        ...
        public static string MD5(string Input)
        {
            return MD5Bytes(ToByteArray(Input));
        }

        public static string MD5(string Input, string EncodingName)
        {
            return MD5Bytes(Encoding.GetEncoding(EncodingName).GetBytes(Input));
        }

        public static string MD5Bytes(byte[] Input)
        {
            MD5CryptoServiceProvider MD5er = new MD5CryptoServiceProvider();
            return ToHex(MD5er.ComputeHash(Input));
        }
        ...
        public static bool HasInvalidUrlCharacters(string _UrlToCheck)
        {
            
            foreach (string  C in UrlInvalidChars)
            {
                if (_UrlToCheck.Contains(C)) return true;
            }
            return false;
        }

        public static byte[] GzipEncode(string Input)
        {
            return GzipEncodeBytes(Encoding.UTF8.GetBytes(Input));
        }

        public static byte[] GzipEncodeBytes(byte[] Input)
        {
        ...
        public static string GzipDecode(byte[] Input)
        {
            return Encoding.UTF8.GetString(GzipDecodeToBytes(Input));
        }

        public static byte[] GzipDecodeToBytes(byte[] Input)
        {
            using (GZipStream GS = new GZipStream(new MemoryStream(Input), CompressionMode.Decompress))
            {
                const int Size = 4096;
        ...
        public static Newtonsoft.Json.Linq.JToken ParseAsJson(string Input)
        {
            try
            {
                return Newtonsoft.Json.Linq.JObject.Parse(Input);
            }
            catch
            {
                return Newtonsoft.Json.Linq.JArray.Parse(Input);
            }
        }
        ...
        public static string BeautifyXml(string XmlInput)
        {
