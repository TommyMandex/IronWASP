...
namespace IronWASP
{
    internal class ScanBranch
    {
        ...
        internal static string LoginRecording = "";
        ...
        internal static void StartBranchScan()
        {
            try
            {
                ...
                if (PickFromProxyLog)
                {
                    IronUI.UpdateScanBranchStats(ScanDone, false, "Searching Proxy Logs for Requests to scan", true, false);
                    foreach (LogRow Row in IronDB.GetRecordsFromProxyLogMatchingFilters(new List<string>() { HostName }, new List<string>() { }, ""))
                    {
                        if (CanScan(Row, "Proxy")) ScanItem(UniqueChecker, "Proxy", Row.ID);
                    }
                }
                if (PickFromProbeLog)
                {
                    IronUI.UpdateScanBranchStats(ScanDone, false, "Searching Probe Logs for Requests to scan", true, false);
                    foreach (LogRow Row in IronDB.GetRecordsFromProbeLogMatchingFilters(new List<string>() { HostName }, new List<string>() { }, ""))
                    {
                        if (CanScan(Row, "Probe")) ScanItem(UniqueChecker, "Probe", Row.ID);
                    }
                }
                ...
                */ 
                ...
                NewRequestSignatures.Clear();
                IronUI.UpdateScanBranchStats(0, true, "All Scan Jobs Created && Queued. Close this Window.", false, true);
            }
        ...
        static Scanner SetRecording(Scanner Scan)
        {
            if (LoginRecording.Length > 0)
            {
                Scan.LoginAndCsrfRecording = Recording.Recording.Get(LoginRecording);
            }
            else
            {
                Scan.LoginAndCsrfRecording = null;
            }
            return Scan;
        }
        ...
        {
            if (!Row.Host.Equals(HostName, StringComparison.OrdinalIgnoreCase)) return false;
            if (UrlPattern.EndsWith("*"))
            {
                string Url = UrlPattern.TrimEnd(new char[] { '*' });
                if (!Row.Url.StartsWith(Url, StringComparison.OrdinalIgnoreCase)) return false;
            }
            else
            {
                if (!Row.Url.Equals(UrlPattern, StringComparison.OrdinalIgnoreCase)) return false;
            }
            if (!HTTPS)
            {
                if ((bool)Row.SSL) return false;
            }
            if (!HTTP)
            {
                if (!(bool)Row.SSL) return false;
