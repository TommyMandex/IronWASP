...
namespace IronWASP
{
    public class IronDB
    {
        ...
        internal static string WorkflowLogFile = "";
        ...
        internal static int LogWorkflow(Workflow.Workflow Flow)
        {
            if (Flow.Id == 0)
            {
                Flow.SetId(Interlocked.Increment(ref Config.WorkflowCount));
            }
            using (SQLiteConnection MT_DB = new SQLiteConnection("data source=" + WorkflowLogFile))
            {
                MT_DB.Open();
                using (SQLiteCommand Cmd = MT_DB.CreateCommand())
                {
                    Cmd.CommandText = "INSERT INTO Workflows (ID, LogIds, LogSource, Name, Type, Source, UserAgent, Info) VALUES (@ID, @LogIds, @LogSource, @Name, @Type, @Source, @UserAgent, @Info)";
                    Cmd.Parameters.AddWithValue("@ID", Flow.Id);
                    Cmd.Parameters.AddWithValue("@LogIds", Flow.GetLogIdsAsString());
                    Cmd.Parameters.AddWithValue("@LogSource", Flow.LogSource);
                    Cmd.Parameters.AddWithValue("@Name", Flow.Name);
                    Cmd.Parameters.AddWithValue("@Type", Flow.FlowType);
                    Cmd.Parameters.AddWithValue("@Source", Flow.FlowSource);
                    Cmd.Parameters.AddWithValue("@UserAgent", Flow.UserAgent);
                    Cmd.Parameters.AddWithValue("@Info", Flow.InfoJson);
                    Cmd.ExecuteNonQuery();
                }
            }
            return Flow.Id;
        ...
        internal static Workflow.Workflow GetWorkflow(int WorkflowId)
        {
            Workflow.Workflow Flow = null;
            using (SQLiteConnection DB = new SQLiteConnection("data source=" + WorkflowLogFile))
            {
                DB.Open();
                using (SQLiteCommand cmd = DB.CreateCommand())
                {
                    cmd.CommandText = "SELECT LogIds, LogSource, Name, Type, Source, UserAgent, Info FROM Workflows WHERE ID=@ID LIMIT 1";
                    cmd.Parameters.AddWithValue("@ID", WorkflowId);
                    using (SQLiteDataReader result = cmd.ExecuteReader())
                    {
                        string Name = result["Name"].ToString();
                        string LogSource = result["LogSource"].ToString();
                        List<int> Ids = Workflow.Workflow.ParseLogIdString(result["LogIds"].ToString());
                        string UserAgent = result["UserAgent"].ToString();
                        string FlowSource = result["Source"].ToString();
                        string FlowType = result["Type"].ToString();
                        string Info = result["Info"].ToString();

                        Workflow.WorkflowSource FlowSourceEnum = (Workflow.WorkflowSource) Enum.Parse(typeof(Workflow.WorkflowSource), FlowSource);
                        Workflow.WorkflowType FlowTypeEnum = (Workflow.WorkflowType) Enum.Parse(typeof(Workflow.WorkflowType), FlowType);

                        Flow = new Workflow.Workflow(Ids, LogSource, UserAgent, FlowSourceEnum, FlowTypeEnum);
                        Flow.SetInfo(JsonConvert.DeserializeObject<Dictionary<string, string>>(Info));
                        Flow.SetName(Name);
                        Flow.SetId(WorkflowId);
                    }
                }
            }
            return Flow;
        ...
        internal static List<int> GetLoginRecordingWorkflowIds()
        {
            ...
            using (SQLiteConnection DB = new SQLiteConnection("data source=" + WorkflowLogFile))
            {
                ...
                using (SQLiteCommand cmd = DB.CreateCommand())
                {
                    cmd.CommandText = "SELECT ID FROM Workflows WHERE Type=@Type";
                    cmd.Parameters.AddWithValue("@Type", Workflow.WorkflowType.Login);
                    using (SQLiteDataReader result = cmd.ExecuteReader())
                    {
                        try
                        {
                            WorkflowIds.Add(Int32.Parse(result["ID"].ToString()));
                        }
                        catch { }
                    }
                }
        ...
        internal static void NameWorkflow(string Name, int WorkflowId)
        {
            using (SQLiteConnection MT_DB = new SQLiteConnection("data source=" + WorkflowLogFile))
            {
                MT_DB.Open();
                using (SQLiteCommand Cmd = MT_DB.CreateCommand())
                {
                    Cmd.CommandText = "UPDATE Workflows SET Name=@Name WHERE ID=@ID";
        ...
        //IronDB.UpdateScan(ScanID, OriginalRequest, "Queued", this.GetInjectionString(), this.BodyFormat.Name, this.GetScanPluginsString(), this.SessionHandler.Name, this.IdOfWorkflowToScan, this.IndexOfRequestToScanInWorkFlowLogAssociations, this.LoginAndCsrfRecording.WorkflowId);
        internal static void UpdateScan(int ScanID, Request Req, string Status, string InjectionPoints, string FormatPlugin, string ScanPlugins, string SessionPlugin, int IdOfWorkflowToScan, int IndexOfRequestToScanInWorkflow, int LoginAndCsrfWorkflowId)
        {
            using(SQLiteConnection DB = new SQLiteConnection("data source=" + IronProjectFile))
            {
                DB.Open();
                using(SQLiteCommand Cmd = DB.CreateCommand())
                {
                    ...
                    Cmd.Parameters.AddWithValue("@WorkflowToScan", JsonConvert.SerializeObject(new Dictionary<string, string>() { { "WorkflowId", IdOfWorkflowToScan.ToString() }, { "RequestIndex", IndexOfRequestToScanInWorkflow.ToString() } }));
                    Cmd.Parameters.AddWithValue("@LoginWorkflow", LoginAndCsrfWorkflowId);
                    Cmd.ExecuteNonQuery();
                }
            }
        }
        ...
        public static List<Dictionary<string, string>> GetUniqueHostsAndSslFromProxyLog()
        {
            return GetUniqueLogFieldValuesFromProxyLog(new List<string>(){"HostName", "SSL"});
        }

        public static List<string> GetUniqueHostsFromProxyLog()
        {
            return GetUniqueLogFieldValuesFromProxyLog("HostName");
        }

        public static List<string> GetUniqueFilesFromProxyLog()
        {
            return GetUniqueLogFieldValuesFromProxyLog("File");
        }

        static List<string> GetUniqueLogFieldValuesFromProxyLog(string FieldName)
        {
            return GetOneFieldValuesOnly(GetUniqueLogFieldValuesFromProxyLog(new List<string>() { FieldName }), FieldName);
        }

        static List<Dictionary<string, string>> GetUniqueLogFieldValuesFromProxyLog(List<string> FieldNames)
        {
            string DataSource = "data source=" + ProxyLogFile;
            string Cmd = string.Format("SELECT DISTINCT {0} FROM ProxyLog", string.Join(", ", FieldNames.ToArray()));
            return GetUniqueLogFieldValuesFromDB(DataSource, FieldNames, Cmd);
        }

        public static List<Dictionary<string, string>> GetUniqueHostsAndSslFromProbeLog()
        {
            return GetUniqueLogFieldValuesFromProbeLog(new List<string>() { "HostName", "SSL" });
        }

        public static List<string> GetUniqueHostsFromProbeLog()
        {
            return GetUniqueLogFieldValuesFromProbeLog("HostName");
        }

        public static List<string> GetUniqueFilesFromProbeLog()
        {
            return GetUniqueLogFieldValuesFromProbeLog("File");
        }

        static List<string> GetUniqueLogFieldValuesFromProbeLog(string FieldName)
        {
            return GetOneFieldValuesOnly(GetUniqueLogFieldValuesFromProbeLog(new List<string>() { FieldName }), FieldName);
        }

        static List<Dictionary<string, string>> GetUniqueLogFieldValuesFromProbeLog(List<string> FieldNames)
        {
            string DataSource = "data source=" + ProbeLogFile;
            string Cmd = string.Format("SELECT DISTINCT {0} FROM ProbeLog", string.Join(", ", FieldNames.ToArray()));
            return GetUniqueLogFieldValuesFromDB(DataSource, FieldNames, Cmd);
        }

        static List<string> GetOneFieldValuesOnly(List<Dictionary<string, string>> FieldValues, string FieldName)
        {
            List<string> Result = new List<string>();
            foreach (Dictionary<string, string> ResultItem in FieldValues)
            {
                try
                {
                    Result.Add(ResultItem[FieldName]);
                }
                catch { }
            }
            return Result;
        ...
        static List<Dictionary<string, string>> GetUniqueLogFieldValuesFromDB(string DataSource, List<string> FieldNames, string CmdString)
        {
            List<Dictionary<string, string>> LogFieldValues = new List<Dictionary<string, string>>();
            using (SQLiteConnection DB = new SQLiteConnection(DataSource))
            {
                DB.Open();
                using (SQLiteCommand cmd = DB.CreateCommand())
                {
                    ...
                    using (SQLiteDataReader result = cmd.ExecuteReader())
                    {
                        while (result.Read())
                        {
                            ...
                            {
                                try { ResultItem[FieldName] = result[FieldName].ToString(); }
                                catch { }
                            }
                            LogFieldValues.Add(ResultItem);
                        }
                    }
                }
            }
        ...
        internal static List<LogRow> GetRecordsFromProxyLogMatchingFilters(List<string> Hosts, List<string> Files, string CsrfTokenParameterName)
        {
            return GetRecordsFromLogMatchingFilters(Hosts, Files, CsrfTokenParameterName, "Proxy");
        }

        internal static List<LogRow> GetRecordsFromProbeLogMatchingFilters(List<string> Hosts, List<string> Files, string CsrfTokenParameterName)
        {
            return GetRecordsFromLogMatchingFilters(Hosts, Files, CsrfTokenParameterName, "Probe");
        }

        static List<LogRow> GetRecordsFromLogMatchingFilters(List<string> Hosts, List<string> Files, string CsrfTokenParameterName, string Source)
        {
            string DataSource = "";
            if (Source.Equals("Proxy"))
            {
                DataSource = "data source=" + ProxyLogFile;
            }
            else if (Source.Equals("Probe"))
            {
                DataSource = "data source=" + ProbeLogFile;
            }
            else
            {
                throw new Exception("Log source must either be Proxy or Probe");
            }

            StringBuilder SB = new StringBuilder(string.Format("SELECT ID , SSL, HostName, Method, URL, File, Parameters, BinaryRequest, Code, Length, MIME, SetCookie, BinaryResponse, RoundTrip FROM {0}Log WHERE (Code != 304) AND ", Source));
            if (Hosts.Count > 0)
            {
                SB.Append(" ( ");
                for (int i = 0; i < Hosts.Count; i++)
                {
                    SB.Append(string.Format("HostName='{0}'", Hosts[i].Replace("'", "\'")));
                    if (i != Hosts.Count - 1) SB.Append(" OR ");
                }
                SB.Append(" ) ");
            }
            else
            {
                SB.Append(" (HostName LIKE '%') ");
            }
            SB.Append(" AND ");
            if (Files.Count > 0)
            {
                ...
                for (int i = 0; i < Files.Count; i++)
                {
                    SB.Append(string.Format("File='{0}'", Files[i].Replace("'", "\'")));
                    if (i != Files.Count - 1) SB.Append(" OR ");
                }
                SB.Append(" ) ");
            }
            else
            ...
            if (CsrfTokenParameterName.Length > 0)
            {
                SB.Append(" AND ");
                SB.Append("(");
                SB.Append(string.Format("URL LIKE '{0}='", Tools.UrlEncode(CsrfTokenParameterName).Replace("'", "\'")));
                SB.Append(" OR ");
                SB.Append("length(RequestBody) > 0");
                SB.Append(")");
            }
            switch(Source)
            {
                case("Proxy"):
                    return GetRecordsFromDB(DataSource, SB.ToString(), "Proxy", 1, Config.LastProxyLogId);
                case("Probe"):
                    return GetRecordsFromDB(DataSource, SB.ToString(), "Probe", 1, Config.LastProbeLogId);
            }
        ...
        public static int GetLastWorkflowId()
        {
            string DataSource = "data source=" + WorkflowLogFile;
            string Cmd = "SELECT max(ID) FROM Workflows";
            return GetLastRowIdFromDB(DataSource, Cmd);
        }
        ...
        internal static Scanner GetScannerFromDB(int ScanID)
        {
            ...
            using (SQLiteConnection DB = new SQLiteConnection("data source=" + IronProjectFile))
            {
                ...
                using (SQLiteCommand cmd = DB.CreateCommand())
                {
                    cmd.CommandText = "SELECT RequestHeaders, RequestBody, BinaryRequest, Status, InjectionPoints, FormatPlugin, SessionPlugin, ScanPlugins, WorkflowToScan, LoginWorkflow FROM ScanQueue WHERE ScanID=@ScanID LIMIT 1";
                    cmd.Parameters.AddWithValue("@ScanID", ScanID);
                    using (SQLiteDataReader result = cmd.ExecuteReader())
                    {
                        IronLogRecord ILR = new IronLogRecord();
                        ...
                        string WorkflowToScan = result["WorkflowToScan"].ToString();
                        string LoginWorkflowIdStr = result["LoginWorkflow"].ToString();
                        ...
                        try
                        {
                            int LoginWorkflowId = Int32.Parse(LoginWorkflowIdStr);
                            Recording.Recording RecordingFromDb = Recording.Recording.FromWorkflow(IronDB.GetWorkflow(LoginWorkflowId));
                            if (Recording.Recording.Has(RecordingFromDb.Name))
                            {
                                ScannerFromLog.LoginAndCsrfRecording = Recording.Recording.Get(RecordingFromDb.Name);
                            }
                            else
                            {
                                ScannerFromLog.LoginAndCsrfRecording = Recording.Recording.FromWorkflow(IronDB.GetWorkflow(LoginWorkflowId));
                                /*
                                if (RecordingFromDb.IsLoginRecordingReplayable())
                                {
                                    ScannerFromLog.LoginAndCsrfRecording = Recording.Recording.FromWorkflow(IronDB.GetWorkflow(LoginWorkflowId));
                                }
                                else
                                {
                                    IronException.Report(string.Format("Recording {0} is not replayable", RecordingFromDb.Name), string.Format("Recording {0} is not replayable. \r\n Please make sure the target is reachable and try loading the recording again.", RecordingFromDb.Name));
                                }
                                */
                            }
                        }
                        ...
                        try
                        {
                            Dictionary<string, string> WorkflowToScanDict = JsonConvert.DeserializeObject<Dictionary<string, string>>(WorkflowToScan);
                            int IdOfWorkflowToScan = Int32.Parse(WorkflowToScanDict["WorkflowId"]);
                            int IndexOfRequestToScan = Int32.Parse(WorkflowToScanDict["RequestIndex"]);
                            Workflow.Workflow FlowToScan = IronDB.GetWorkflow(IdOfWorkflowToScan);
                            ScannerFromLog.IdOfWorkflowToScan = IdOfWorkflowToScan;
                            ScannerFromLog.IndexOfRequestToScanInWorkFlowLogAssociations = IndexOfRequestToScan;
                            ScannerFromLog.WorkFlowLogAssociations = FlowToScan.WorkflowAssociations;
                        }
                        catch { }
        ...
        internal static void CreateNewLogFiles()
        {
            using (SQLiteConnection log = new SQLiteConnection("data source=" + IronProjectFile))
            {
                log.Open();
                using (SQLiteTransaction create = log.BeginTransaction())
                {
                    using (SQLiteCommand cmd = new SQLiteCommand(log))
                    {
                        ...
                        cmd.ExecuteNonQuery();
                        cmd.CommandText = "CREATE TABLE IF NOT EXISTS ScanQueue (ScanID INT PRIMARY KEY, RequestHeaders TEXT, RequestBody TEXT, BinaryRequest INT, Status TEXT, Method TEXT, URL TEXT, SessionPlugin TEXT, InjectionPoints TEXT, FormatPlugin TEXT, ScanPlugins TEXT, WorkflowToScan TEXT, LoginWorkflow TEXT)";
                        cmd.ExecuteNonQuery();
            ...
            using (SQLiteConnection log = new SQLiteConnection("data source=" + WorkflowLogFile))
            {
                log.Open();
                using (SQLiteTransaction create = log.BeginTransaction())
                {
                    using (SQLiteCommand cmd = new SQLiteCommand(log))
                    {
                        cmd.CommandText = "DROP TABLE IF EXISTS Workflows";
                        cmd.ExecuteNonQuery();
                        cmd.CommandText = "CREATE TABLE IF NOT EXISTS Workflows (ID INT PRIMARY KEY, LogIds TEXT, LogSource TEXT, Name TEXT, Type TEXT, Source TEXT, UserAgent TEXT, Info TEXT)";
                        cmd.ExecuteNonQuery();
                    }
                    create.Commit();
                }
        ...
        internal static void MakeLogFileVersionCompliant()
        {
            ...
            using (SQLiteConnection DB = new SQLiteConnection("data source=" + IronProjectFile))
            {
                ...
                try
                {
                    using (SQLiteCommand Cmd = DB.CreateCommand())
                    {
                        Cmd.CommandText = "SELECT WorkflowToScan FROM ScanQueue WHERE ScanID=1";
                        using (SQLiteDataReader result = Cmd.ExecuteReader())
                        {
                            bool HasRows = result.HasRows;
                        }
                ...
                catch
                {
                    try
                    {
                        using (SQLiteCommand Cmd = DB.CreateCommand())
                        {
                            Cmd.CommandText = "ALTER TABLE ScanQueue ADD COLUMN WorkflowToScan TEXT";
                            Cmd.ExecuteNonQuery();
                            Cmd.CommandText = "ALTER TABLE ScanQueue ADD COLUMN LoginWorkflow TEXT";
                            Cmd.ExecuteNonQuery();
                        }
                    }
                    catch { }
        ...
        internal static void UpdateLogFilePaths(string _LogPath)
        {
            IronDB.LogPath = _LogPath;
            IronProjectFile = _LogPath + "\\Project.iron";
            ProxyLogFile = _LogPath + "\\Proxy.ironlog";
            TestLogFile = _LogPath + "\\Test.ironlog";
            ShellLogFile = _LogPath + "\\Shell.ironlog";
            ProbeLogFile = _LogPath + "\\Probes.ironlog";
            ScanLogFile = _LogPath + "\\Scan.ironlog";

            WorkflowLogFile = _LogPath + "\\Workflow.ironlog";

            PluginResultsLogFile = _LogPath + "\\Results.ironlog";
            ExceptionsLogFile = _LogPath + "\\Exceptions.ironlog";
            TraceLogFile = _LogPath + "\\Trace.ironlog";
            ...
            CommandsLogFile = new StreamWriter(_LogPath + "\\CommandLog.txt", true);
        }
