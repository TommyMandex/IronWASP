...
namespace IronWASP
{
    ...
    {
        public const string NormalStringContext = "NormalString";
        public const string DoubleQuotedStringContext = "DoubleQuotedString";
        public const string SingleQuotedStringContext = "SingleQuotedString";
        public const string SingleLineCommentContext = "SingleLineComment";
        public const string MultiLineCommentContext = "MutliLineComment";

        public static Dictionary<string, string> ContextDescriptions = new Dictionary<string, string>() { 
            {NormalStringContext, "Executable section of code"},
            {DoubleQuotedStringContext, "Inside double quotes"},
            {SingleQuotedStringContext, "Inside single quotes"},
            {SingleLineCommentContext, "Inside a single-line comment"},
            {MultiLineCommentContext, "Inside a multi-line comment"}
        };
            ...
            while(CharPointer < CodeChars.Length)
            {
                ...
                switch(CurrentChar)
                {
                    case ('"'):
                        ...
                        CheckAndAddContext(NormalString, Keyword, NormalStringContext);
                        CharPointer++;
                        ReadTill("\"", false, true);
                        string DoubleQuotedString = new string(CurrentTokenChars.ToArray());
                        if (DoubleQuotedString.EndsWith("\""))
                        {
                            CheckAndAddContext(DoubleQuotedString.TrimEnd('"'), Keyword, DoubleQuotedStringContext);
                        }
                        else
                        {
                            CheckAndAddContext(DoubleQuotedString, Keyword, NormalStringContext);
                        }
                        break;
                    case ('\''):
                        NormalString = new string(CurrentTokenChars.ToArray());
                        ...
                        {
                            CheckAndAddContext(SingleQuotedString.TrimEnd('\''), Keyword, SingleQuotedStringContext);
                        }
                        else
                        {
                            CheckAndAddContext(SingleQuotedString, Keyword, NormalStringContext);
                        }
                        break;
                    case ('/'):
                        if (CharPointer < CodeChars.Length - 1)
                        {
                            if (CodeChars[CharPointer + 1] == '/')
                            {
                                ...
                                CheckAndAddContext(SingleLineComment, Keyword, SingleLineCommentContext);
                            }
                            else if (CodeChars[CharPointer + 1] == '*')
                            {
                                ...
                                CheckAndAddContext(MutliLineComment, Keyword, MultiLineCommentContext);
                            }
                            else
                            {
