...
namespace IronWASP
{
    public class Response
    {
        ...
        public static Response FromBinaryString(string BinaryResponseString)
        {
            string[] ResponseParts = BinaryResponseString.Split(new char[] { ':' }, 2, StringSplitOptions.RemoveEmptyEntries);
            if (ResponseParts.Length == 1)
            {
                return new Response(Tools.Base64Decode(ResponseParts[0]));
            }
            else if (ResponseParts.Length == 2)
            {
                return new Response(Tools.Base64Decode(ResponseParts[0]), Tools.Base64DecodeToByteArray(ResponseParts[1]));
            }
            else
            {
