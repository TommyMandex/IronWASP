    ...
    public class IronUI
    {
        ...
        internal static void UpdateUIFromConfig()
        {
            ...
            else
            {
                ...
                UpdateJavaScriptTraceSettingsInUiFromConfig();
            }
        }
        ...
        internal static void UpdateJavaScriptTraceSettingsInUiFromConfig()
        {
            UI.JsTraceMonitorEvalCB.Checked = JavaScriptTracer.MonitorEval;
            UI.JsTraceMonitorSetTimeoutCB.Checked = JavaScriptTracer.MonitorSetTimeout;
            UI.JsTraceMonitorSetIntervalCB.Checked = JavaScriptTracer.MonitorSetInterval;
            UI.JsTraceMonitorFunctionMethodCB.Checked = JavaScriptTracer.MonitorFunctionMethods;
            UI.JsTraceMonitorNonNativeMethodCB.Checked = JavaScriptTracer.MonitorUserDefinedMethods;

            UI.JsTraceMonitorInnerHtmlCB.Checked = JavaScriptTracer.MonitorInnerHtmlAssignments;
            
            UI.JsTraceMonitorXhrCB.Checked = JavaScriptTracer.MonitorXhr;
        }
        ...
        delegate void StartUpdatingFullUIFromDB_d();
        internal static void StartUpdatingFullUIFromDB()
        {
            if (UI.InvokeRequired)
            {
                StartUpdatingFullUIFromDB_d CALL_d = new StartUpdatingFullUIFromDB_d(StartUpdatingFullUIFromDB);
                UI.Invoke(CALL_d, new object[] { });
            }
            else
            {
        ...
        static void ShowWaitForm()
        {
            ...
            IronUI.WF.WaitFormProgressBar.Maximum = 13;
            ...
            IronUI.WF.ProjectLoadGrid.Rows.Add(new object[] { "Load Recordings", "0", "Not Done" });
        ...
        internal static void UpdateFullUIFromDB()
        {
            ...
            ShowWaitFormMessage("Loading Recordings from Log...");
            try
            {
                Config.WorkflowCount = IronDB.GetLastWorkflowId();
            }
            catch { }

            ShowWaitFormGridMessage(12, Counter, "In Progress", 2, false);
            Recording.Recording.ClearAll();
            Counter = 0;
            try
            {
                List<int> RecIds = IronDB.GetLoginRecordingWorkflowIds();
                foreach (int Id in RecIds)
                {
                    try
                    {
                        Recording.Recording Rec = Recording.Recording.FromWorkflow(IronDB.GetWorkflow(Id));
                        Recording.Recording.Add(Rec);
                        Counter++;
                    }
                    catch { }
                }
                ShowWaitFormGridMessage(12, Counter, "Done", 1, false);
            }
            catch 
            {
                ShowWaitFormMessage("Error loading Recordings");
                ShowWaitFormGridMessage(12, Counter, "Failed", 3, false);
            }
        ...
        delegate void UpdateScanBranchStats_d(int ScanDone, bool AllScansStarted, string Message, bool Progress, bool CloseWindow);
        internal static void UpdateScanBranchStats(int ScanDone, bool AllScansStarted, string Message, bool Progress, bool CloseWindow)
        {
            if (IronUI.SBF.InvokeRequired)
            {
                UpdateScanBranchStats_d USBS_d = new UpdateScanBranchStats_d(UpdateScanBranchStats);
                UI.Invoke(USBS_d, new object[] { ScanDone, AllScansStarted, Message, Progress, CloseWindow});
            }
            else
            {
                ...
                if (AllScansStarted)
                {
                    IronUI.SBF.FinalBtn.Text = "Close";
                    IronUI.SBF.ScanBranchProgressBar.Visible = false;
                }
        ...
        internal static void ShowCurrentLogStat()
        {
            ...
            else
            {
                try
                {
                    ...
                    {
                        case ("ProxyLogTab"):
                            UI.MainLogStatLbl.Text = string.Format("Showing {0} - {1} of {2} Proxy Logs", IronLog.ProxyMin, IronLog.ProxyMax, Config.LastProxyLogId);
                            break;
                        case ("ScanLogTab"):
                            UI.MainLogStatLbl.Text = string.Format("Showing {0} - {1} of {2} Scan Logs", IronLog.ScanMin, IronLog.ScanMax, Config.LastScanLogId);
                            break;
                        case ("TestLogTab"):
                            UI.MainLogStatLbl.Text = string.Format("Showing {0} - {1} of {2} Test Logs", IronLog.TestMin, IronLog.TestMax, Config.LastTestLogId);
                            break;
                        case ("ShellLogTab"):
                            UI.MainLogStatLbl.Text = string.Format("Showing {0} - {1} of {2} Shell Logs", IronLog.ShellMin, IronLog.ShellMax, Config.LastShellLogId);
                            break;
                        case ("ProbeLogTab"):
                            UI.MainLogStatLbl.Text = string.Format("Showing {0} - {1} of {2} Probe Logs", IronLog.ProbeMin, IronLog.ProbeMax, Config.LastProbeLogId);
                            break;
                        case ("OtherLogTab"):
                            if (Config.GetOtherSourceList().Contains(IronLog.SelectedOtherSource))
                            {
                                UI.MainLogStatLbl.Text = string.Format("Showing {0} - {1} of  {2} {3} Logs", IronLog.OtherSourceMin, IronLog.OtherSourceMax, Config.GetLastLogId(IronLog.SelectedOtherSource), IronLog.SelectedOtherSource);
                            }
                            break;
                        case ("SiteMapLogTab"):
                ...
                catch (Exception Exp) { IronException.Report("Error showing log stats", Exp); }
            }
        }
        ...
        delegate void ShowJsTraceResult_d(string Msg);
        internal static void ShowJsTraceResult(string Msg)
        {
            if (UI.InvokeRequired)
            {
                UI.Invoke(new ShowJsTraceResult_d(ShowJsTraceResult), new object[] { Msg });
            }
            else
            {
                StringBuilder SB = new StringBuilder(@"{\rtf1{\colortbl ;\red0\green77\blue187;\red247\green150\blue70;\red255\green0\blue0;\red0\green200\blue50;}");
                SB.Append(Tools.RtfSafe(Msg));
                UI.JsTraceResultsRTB.Rtf = SB.ToString();
            }
        }

        delegate void AddJsTraceObservation_d(List<JavaScriptTracer.ObservationMsg> Msgs);
        internal static void AddJsTraceObservation(List<JavaScriptTracer.ObservationMsg> Msgs)
        {
            if (UI.InvokeRequired)
            {
                UI.Invoke(new AddJsTraceObservation_d(AddJsTraceObservation), new object[] { Msgs });
            }
            else
            {
                if (Msgs.Count > 0)
                {
                    ...
                    {
                        UI.JsTraceResultsObservationsGrid.Rows.Add(new object[] { UI.JsTraceResultsObservationsGrid.Rows.Count + 1, Msg.Type, Msg.Url, Msg.Details });
                    }
                    UI.JsTraceResultsTabs.TabPages["JsTraceResultsObservationsTab"].Text = string.Format("    Observations  - {0}  ", UI.JsTraceResultsObservationsGrid.Rows.Count);
                }
            }
        }

        delegate void AddJsTraceNativeMethodCall_d(List<JavaScriptTracer.NativeMethodCallMsg> Msgs);
        internal static void AddJsTraceNativeMethodCall(List<JavaScriptTracer.NativeMethodCallMsg> Msgs)
        {
            if (UI.InvokeRequired)
            {
                UI.Invoke(new AddJsTraceNativeMethodCall_d(AddJsTraceNativeMethodCall), new object[] { Msgs });
            }
            else
            {
                if (Msgs.Count > 0)
                {
                    foreach (var Msg in Msgs)
                    {
                        UI.JsTraceResultsMethodCallsGrid.Rows.Add(new object[] { UI.JsTraceResultsMethodCallsGrid.Rows.Count + 1, "", Msg.Time, Msg.Origin, Msg.Method, Msg.Url, Msg.Details });
                    }
                    UI.JsTraceResultsTabs.TabPages["JsTraceResultsNativeMethodsTab"].Text = string.Format("    Native Method Calls  - {0}  ", UI.JsTraceResultsMethodCallsGrid.Rows.Count);
                }
            }
        }

        delegate void AddJsTraceDomChangeCall_d(List<JavaScriptTracer.DomChangeMsg> Msgs);
        internal static void AddJsTraceDomChangeCall(List<JavaScriptTracer.DomChangeMsg> Msgs)
        {
            if (UI.InvokeRequired)
            {
                UI.Invoke(new AddJsTraceDomChangeCall_d(AddJsTraceDomChangeCall), new object[] { Msgs });
            }
            else
            {
                if (Msgs.Count > 0)
                {
                    foreach (var Msg in Msgs)
                    {
                        UI.JsTraceResultsDomChangesGrid.Rows.Add(new object[] { UI.JsTraceResultsDomChangesGrid.Rows.Count + 1, "", Msg.Time, Msg.Origin, Msg.Action, Msg.Url, Msg.Details });
                    }
                    UI.JsTraceResultsTabs.TabPages["JsTraceResultsDomChangesTab"].Text = string.Format("    Dom Changes  - {0}  ", UI.JsTraceResultsDomChangesGrid.Rows.Count);
                }
            }
        }

        delegate void AddJsTraceAjaxCall_d(List<JavaScriptTracer.AjaxCallMsg> Msgs);
        internal static void AddJsTraceAjaxCall(List<JavaScriptTracer.AjaxCallMsg> Msgs)
        {
            if (UI.InvokeRequired)
            {
                UI.Invoke(new AddJsTraceAjaxCall_d(AddJsTraceAjaxCall), new object[] { Msgs });
            }
            else
            {
                if (Msgs.Count > 0)
                {
                    foreach (var Msg in Msgs)
                    {
                        UI.JsTraceResultsAjaxCallsGrid.Rows.Add(new object[] { UI.JsTraceResultsAjaxCallsGrid.Rows.Count + 1, "", Msg.Time, Msg.SourceOrigin, Msg.TargetOrigin, Msg.Reflection, Msg.Url, Msg.Details });
                    }
                    UI.JsTraceResultsTabs.TabPages["JsTraceResultsAjaxCallsTab"].Text = string.Format("    Ajax Calls  - {0}  ", UI.JsTraceResultsAjaxCallsGrid.Rows.Count);
                }
            }
        }
