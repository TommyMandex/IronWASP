    ...
    public class CrossSiteScriptingCheck : ActivePlugin
    {
        ...
        void CheckForInjectionInJS(bool InLine)
        {
            ...
            if (!js_inj_success)
            {
                ...
                if (script_contexts.Contains(CodeContext.SingleQuotedStringContext))
                {
                    payload_prefixes.Add("'");
                }
                if (script_contexts.Contains(CodeContext.DoubleQuotedStringContext))
                {
                    payload_prefixes.Add("\"");
                }
                if (script_contexts.Contains(CodeContext.SingleLineCommentContext))
                {
            ...
            if (!js_inj_success)
            {
                if (script_contexts.Contains(CodeContext.NormalStringContext))
                {
                    js_inj_success = true;
                }
            }
        ...
        void CheckForInjectionInVBInsideHTML()
        {
            ...
            foreach (string pp in payload_prefixes)
            {
                ...
                {
                    this.Scnr.ResponseTrace(string.Format("Could not get {0} as VB Script statement", keyword));
                }
            }
        }
