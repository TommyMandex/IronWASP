    ...
    public class SqlInjectionCheck : ActivePlugin
    {
        ...
        int InjectBlindIntMath(int int_value)
        {
            ...
            */
            ...
            int[] minus_left = new int[] { val + 101, val + 102, sub_val + 100, sub_val + 101, val, val };
            string[] minus_right = new string[] { "101", "102", "105", "106", "a", "b" };
            ...
            foreach (string quote in quotes)
            {
                ...
                for (int j = 0; j < 2; j++)
                {
                    ...
                    else
                    {
                        ...
                        if (first_strict_vuln && first_strict_signature == second_strict_signature)
                        {
                            this.Scnr.Trace("  <i<b>>Even the second time Strict Grouping indicates that injection succeeded.<i</b>>");
                            if (DoesSignaturesMatchInvalidStringConcCheck(str_value_first_part, quote, joiners, str_value_second_part, keys, new string[] { second_strict_signature, second_relaxed_signature }))
                            {
                                this.Scnr.Trace("  <i<b>>Both valid and invalid string concatenation attempts provide the same type of Strict Grouping, so this is a false alarm.<i</b>>");
                            }
                            else
                            {
                                vuln = true;
                                confidence = confidence + 1;
                                this.Scnr.Trace("  <i<cr>>Strict Grouping did not work for invalid concatenation attempt, indicates that the grouping was because of successful injection.<i</cr>>");
                        ...
                        if (first_relaxed_vuln && first_relaxed_signature == second_relaxed_signature)
                        {
                            this.Scnr.Trace("  <i<b>>Even the second time Relaxed Grouping indicates that injection succeeded.<i</b>>");
                            if (DoesSignaturesMatchInvalidStringConcCheck(str_value_first_part, quote, joiners, str_value_second_part, keys, new string[] { second_strict_signature, second_relaxed_signature }))
                            {
                                this.Scnr.Trace("  <i<b>>Both valid and invalid string concatenation attempts provide the same type of Relaxed Grouping, so this is a false alarm.<i</b>>");
                            }
                            else
                            {
                                vuln = true;
                                confidence = confidence + 1;
                                this.Scnr.Trace("  <i<cr>>Relaxed Grouping did not work for invalid concatenation attempt, indicates that the grouping was because of successful injection.<i</cr>>");
        ...
        bool DoesSignaturesMatchInvalidStringConcCheck(string str_value_first_part, string quote, string[] joiners, string str_value_second_part, string[] keys, string[] Signatures)
        {
            List<string> payloads = new List<string>();
            List<Request> requests = new List<Request>();
            List<Response> responses = new List<Response>();
            ...
            SimilarityChecker sc = new SimilarityChecker();
            this.Scnr.Trace("<i<br>><i<b>>Checking if the grouping occurs for invalid concatenation attempts. This is done to eliminate false positives.<i</b>>");
            this.Scnr.Trace(string.Format("Removing one {0} from the two used in the valid concatenation attempt.", quote));
            for (int i = 0; i < 3; i++)
            {
                string payload = string.Format("{0}{1}{2}{3}", str_value_first_part, joiners[i], quote, str_value_second_part);
                this.Scnr.RequestTrace(string.Format("  Request Key: '{0}' - Injecting {1}", keys[i], payload));
                Response res = this.Scnr.Inject(payload);
                payloads.Add(payload);
                requests.Add(this.Scnr.InjectedRequest.GetClone());
            ...
            sc.Check();
            if (IsBlindStrConcInjectableGroupingCheck(sc.StrictGroups) && sc.StrictGroupsSignature == Signatures[0])
            {
                return true;
            }
            if (IsBlindStrConcInjectableGroupingCheck(sc.RelaxedGroups) && sc.RelaxedGroupsSignature == Signatures[1])
            {
                return true;
            }
            return false;
        ...
        {
            string Reason = "IronWASP sent six payloads to the application with SQL code snippets in them.<i<br>>";
        ...
        FindingReason GetBlindConcatReason(List<string> payloads, string db, int Trigger)
        {
            ...
            Reason = Reason + string.Format("Since incase of <i<hlg>>{0}<i</hlg>> database server Payloads {1} & {2} would have simply thrown an invalid SQL syntax exception their responses are similar. ", db, same[0], same[1]);
