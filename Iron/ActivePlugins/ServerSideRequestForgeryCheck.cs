    ...
    public class ServerSideRequestForgeryCheck : ActivePlugin
    {
        ...
        void CheckForSSRF()
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                string payload_a1 = string.Format("{0}localhost:65533", p);
                ...
                string payload_b1 = string.Format("{0}localhost:65534", p);
                ...
                request_trigger_descs.Add(string.Format("This payload points to the valid port 65533 on localhost. The payload is {0}", payload_a1));
                ...
                request_trigger_descs.Add(string.Format("This payload points to the valid port 65534 on localhost. The payload is {0}", payload_b1));
        ...
        FindingReason GetDiffReason(List<string> payloads, bool time, List<int> time_delays, int trigger_start, int trigger_count)
        {
            ...
            Reason = Reason + "<i<br>>Payloads B and D are similar to each other but different from A & C. They both refer to ports 65533 and 65534 on the server which are valid ports.";
            Reason = Reason + "<i<br>>If the application is vulnerable to SSRF then it will try to connect to these ports and connections to invalid ports with throw an exception of different type than the exception or error caused by connecting to the valid ports 65533 and 65534 which are most likely to be closed.";
