...
namespace IronWASP
{
    public class HTML
    {
        ...
        public static bool DoesHaveAngleBracketsAsStartingBoundary(string Input)
        {
        ...
        public List<string> GetDecodedValues(string ElementName, string AttributeName)
        {
            List<string> DecodedValues = new List<string>();
            foreach(string Value in QueryValues(string.Format("//{0}[@{1}]", ElementName.ToLower(), AttributeName.ToLower()), AttributeName.ToLower()))
            {
                DecodedValues.Add(Tools.HtmlDecode(Value));
            }
        ...
        public List<string> GetDecodedValues(string ElementName, string AttributeName, string AttributeValue, string InterestedAttributeName)
        {
            List<string> DecodedValues = new List<string>();
            foreach (string Value in QueryValues(string.Format("//{0}[@{1}={2}]", ElementName.ToLower(), AttributeName.ToLower(), XpathSafe(AttributeValue)), InterestedAttributeName.ToLower()))
            {
                DecodedValues.Add(Tools.HtmlDecode(Value));
            }
            return DecodedValues;
        ...
        public List<string> GetValuesIgnoreValueCase(string ElementName, string AttributeName, string AttributeValue, string InterestedAttributeName)
        {
            ...
            List<HtmlNode> Nodes = GetNodes(ElementName, AttributeName);
        ...
        public List<HtmlNode> GetNodes(string ElementName)
        {
            return QueryNodes(string.Format("//{0}", ElementName.ToLower()));
        }

        public List<HtmlNode> GetNodes(string ElementName, string AttributeName)
        {
            return QueryNodes(string.Format("//{0}[@{1}]", ElementName.ToLower(), AttributeName.ToLower()));
        }

        public List<HtmlNode> GetNodes(string ElementName, string AttributeName, string AttributeValue)
        {
            return QueryNodes(string.Format("//{0}[@{1}={2}]", ElementName.ToLower(), AttributeName.ToLower(), XpathSafe(AttributeValue)));
        }

        public List<HtmlNode> QueryNodes(string Xpath)
        {
            return NodeCollectionToNodeList(Html.DocumentNode.SelectNodes(Xpath));
        }
        ...
        public List<string> Query(string Xpath)
        {
            ...
            try
            {
                List<HtmlNode> Nodes = QueryNodes(Xpath);
                foreach (HtmlNode Node in Nodes)
                {
        ...
        public List<HtmlNode> GetLinks()
        {
            ...
            List<HtmlNode> LinkNodeCollection = this.GetNodes("a", "href");            
        ...
        public List<IronHtml.LinkElement> GetLinkElements()
        {
            List<IronHtml.LinkElement> LinkElements = new List<IronWASP.IronHtml.LinkElement>();

            List<HtmlNode> LinkNodes = GetLinks();

            for (int i=0; i < LinkNodes.Count; i++)
            {
                LinkElements.Add(new IronHtml.LinkElement(LinkNodes[i], i));
            }
        ...
        public List<HtmlNode> GetForms()
        {
            ...
            List<HtmlNode> Forms = this.GetNodes("form");
        ...
        public List<IronHtml.FormElement> GetFormElements()
        {
            List<IronHtml.FormElement> FormElements = new List<IronHtml.FormElement>();

            List<HtmlNode> FormNodes = GetForms();

            for (int i=0; i < FormNodes.Count; i++)
            {
                FormElements.Add(new IronHtml.FormElement(FormNodes[i], i));
            }
        ...
        {
            List<string> Scripts = GetScriptFromScriptTags(Keyword, ShouldGetJavaScript);

            if (ShouldGetJavaScript)
                Scripts.AddRange(GetJavaScriptFromAttributes(Keyword));
            else
                Scripts.AddRange(GetVisualBasicFromAttributes(Keyword));
            return Scripts;
        }

        public List<string> GetJavaScriptFromScriptTags()
        {
            return GetJavaScriptFromScriptTags("");
        }

        public List<string> GetJavaScriptFromScriptTags(string Keyword)
        {
            return GetScriptFromScriptTags(Keyword, true);
        }

        public List<string> GetVisualBasicFromScriptTags()
        {
            return GetVisualBasicFromScriptTags("");
        }

        public List<string> GetVisualBasicFromScriptTags(string Keyword)
        {
            return GetScriptFromScriptTags(Keyword, false);
        }

        List<string> GetScriptFromScriptTags(string Keyword, bool ShouldGetJavaScript)
        {
            List<string> Scripts = new List<string>();
            List<HtmlNode> JSNodes = this.GetNodes("script");
        ...
        public List<string> GetJavaScriptFromAttributes()
        {
            return GetJavaScriptFromAttributes("");
        }

        public List<string> GetJavaScriptFromAttributes(string Keyword)
        {
            return GetJavaScript(this.Html.DocumentNode, Keyword);
        }

        public List<string> GetVisualBasicFromAttributes()
        {
            return GetVisualBasicFromAttributes("");
        }

        public List<string> GetVisualBasicFromAttributes(string Keyword)
        {
            return GetVisualBasic(this.Html.DocumentNode, Keyword);
        }
        ...
        public List<string> GetCss(string Keyword, bool WrapInLineCss)
        {
            ...
            List<HtmlNode> CssNodes = this.GetNodes("style");
        ...
        public static List<HtmlNode> NodeCollectionToNodeList(HtmlNodeCollection NodeCollec)
        {
            List<HtmlNode> Nodes = new List<HtmlNode>();
            if (NodeCollec != null)
            {
                foreach (HtmlNode Node in NodeCollec)
                {
                    Nodes.Add(Node);
                }
            }
