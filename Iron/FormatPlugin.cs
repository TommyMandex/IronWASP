    ...
    public class FormatPlugin : Plugin
    {
        ...
        public static string[,] XmlToArray(string XML)
        {
            ...
            while (Read)
            {
                ...
                if (Reader.IsStartElement())
                {
                    ...
                    if (HasOname)
                    {
                        ...
                        {
                            if (Reader.GetAttribute("oname") != null && Reader.GetAttribute("oname").Length > 0)
                            {
                                PropertyPaths.Add(Reader.GetAttribute("oname"));
                            }
                            else
                            {
            ...
            for (int i = 0; i < ParameterNames.Count; i++)
            {
                ...
                if (IsOldVersion)
                {
                    InjectionPoints[i, 1] = Tools.Base64Encode(ParameterValues[i]);
                }
                else
                {
        ...
        public static FormatPlugin Get(BodyFormatType BodyType)
        {
            switch(BodyType)
            {
                case(BodyFormatType.Json):
                    return Get("JSON");
                case (BodyFormatType.Xml):
                    return Get("XML");
                case (BodyFormatType.Multipart):
                    return Get("MultiPart");
                case (BodyFormatType.Soap):
                    return Get("SOAP");
            }
            return null;
        }
        ...
        public virtual void WriteValue(XmlWriter XW, string Value)
        {
            if (Value.Contains("\r\n"))
            {
                XW.WriteAttributeString("crlf", "1");
            }
            XW.WriteValue(Tools.Base64Encode(Value));
        }

        public virtual void WriteValue(XmlWriter XW, byte[] Value)
        {
            XW.WriteValue(Tools.Base64EncodeByteArray(Value));
        }
        ...
        public static FormatPlugin JSON()
        {
            return Get("JSON");
        }
        public static FormatPlugin XML()
        {
            return Get("XML");
        }
        public static FormatPlugin MutliPart()
        {
            return Get("MultiPart");
        }
        public static FormatPlugin SOAP()
        {
            return Get("SOAP");
        }

        public static FormatParameters GetXmlParameters(Request Req)
        {
            return new FormatParameters(XmlToArray(FormatPlugin.XML().ToXmlFromRequest(Req)));
        }
        ...
        public static FormatParameters GetXmlParameters(Response Res)
        {
            return new FormatParameters(XmlToArray(FormatPlugin.XML().ToXmlFromResponse(Res)));
        }
